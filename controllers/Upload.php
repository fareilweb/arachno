...
class Upload extends Controller
{
    ...
    public function saveFile($tmp_name=NULL, $relative_target=NULL){
        $absolute_target = Config::$abs_path . $relative_target;
    ...
    function itemImagesProcess($args)
    {
        ...
        if(isset($this->files[$this->inputName]) && count($this->files[$this->inputName])>0)
        {
            ...
            { 
                ...
                $file->error    = $errors[$i];
                $file->size     = $sizes[$i];
                $save_target    = "/views/images/shop/items/" . $file->name;
